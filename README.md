...
| `shouldStart`              | `(firstMove: Position, info: { pointerType: string }) => boolean` | Optional escape hatch. Evaluated on the first pointermove past a few-pixel threshold. Return `false` to abandon the gesture so native behavior can continue, `true` to take over as a drag. When omitted, the hook auto-detects: on `pointerdown` it walks from the event target up to the drag root looking for a scrollable element; on touch/pen input it then defers to native scroll until the scroll container hits an edge in the gesture's direction (the standard bottom-sheet pattern). Mouse always drags immediately. Set `touch-action: pan-x` / `pan-y` on the scrollable element so the browser performs the native scroll. |
