...
### Custom `shouldStart` with a scrollable subtree

`shouldStart` and scroll auto-detection are independent: `shouldStart` answers _is this a drag?_, while the scrollable lookup answers _where does the gesture go when it isn't?_ The two compose — if your `shouldStart` returns `false` and there's a scrollable descendant under the gesture, the hook takes over the scroll itself (momentum and all), exactly as in the auto-detect case. Keep `touch-action: none` on both elements (the same CSS as above); do **not** use `pan-y` here, or the browser will fight the hook for the gesture and may cancel it mid-drag.

### Custom `shouldStart` without a scrollable subtree

The `pan-y` fallback below applies **only** when there's no scrollable descendant for the hook to defer to. In that case returning `false` releases the pointer and lets the browser take over — set `touch-action` to whatever native gesture you want as the fallback (`pan-y` for vertical scroll, `pan-x` for horizontal, etc.):
